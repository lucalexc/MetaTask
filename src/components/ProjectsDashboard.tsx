...
export default function ProjectsDashboard() {
  ...
  if (isLoading) {
    return (
      <div className="flex-1 flex items-center justify-center bg-white">
        <Loader2 className="w-8 h-8 animate-spin text-[#DC4C3E]" />
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col bg-white h-screen overflow-hidden">
      <header className="px-4 md:px-8 py-4 md:py-6 border-b border-gray-100 flex justify-between items-center shrink-0">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Meus Projetos</h1>
          <span className="px-2.5 py-1 rounded-full bg-gray-100 text-xs font-medium text-gray-500 border border-gray-200">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-[#DC4C3E] text-white text-sm font-medium rounded-lg hover:bg-[#C53727] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
        ...
        <div className="max-w-5xl mx-auto">
          {projects.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-gray-400 mb-6">Crie seu primeiro projeto para organizar suas tarefas.</p>
              <button
                onClick={() => setIsModalOpen(true)}
                className="inline-flex items-center gap-2 px-4 py-2 bg-[#DC4C3E] text-white text-sm font-medium rounded-lg hover:bg-[#C53727] transition-colors"
              >
                <Plus className="w-4 h-4" />
                Criar Projeto
              </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {projects.map((project) => (
                  <div
                    ...
                    className="group relative bg-white border border-gray-200 rounded-xl p-5 hover:border-gray-300 hover:shadow-md transition-all cursor-pointer overflow-hidden animate-in fade-in zoom-in-95 duration-200"
                  >
                    {/* Colored sidebar */}
                    <div 
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-bold text-gray-900 leading-none">{project.name}</h3>
                          {project.description && (
...
function NewProjectModal({ onClose, onSave }: { onClose: () => void; onSave: (name: string, description: string, color: string) => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div
        className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden border border-gray-200 animate-in zoom-in-95 duration-200"
      >
        <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center bg-white">
          <h3 className="font-semibold text-base text-gray-900">Novo Projeto</h3>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 p-1 rounded-md hover:bg-gray-50 transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-5">
          <div>
            <label className="block text-[11px] uppercase tracking-wider font-bold text-gray-400 mb-1.5">Nome do Projeto</label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="Ex: Trabalho, Pessoal..."
              className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#DC4C3E]/10 focus:border-[#DC4C3E] font-medium transition-all"
              autoFocus
            />
          </div>

          <div>
            <label className="block text-[11px] uppercase tracking-wider font-bold text-gray-400 mb-1.5">Descrição</label>
            <textarea
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="Opcional..."
              rows={2}
              className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#DC4C3E]/10 focus:border-[#DC4C3E] text-sm resize-none transition-all"
            />
          </div>

          <div>
            <label className="block text-[11px] uppercase tracking-wider font-bold text-gray-400 mb-2.5">Cor do Projeto</label>
        ...
        <div className="px-5 py-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-2">
          ...
          <button
            ...
            disabled={!name.trim()}
            className="px-4 py-1.5 bg-[#DC4C3E] text-white text-xs font-bold rounded-md hover:bg-[#C53727] transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
          >
            Criar Projeto
          </button>
        </div>
    ...
    <div className="flex-1 flex flex-col bg-white h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8">
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
          {/* Left Column: Tasks */}
          <div className="bg-gray-50 rounded-2xl p-6 border border-gray-100 flex flex-col h-[600px]">
            <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4 shrink-0">Tarefas Abertas</h2>
            ...
            <div className="flex-1 overflow-y-auto pr-2 space-y-3">
              {isLoadingTasks ? (
                ...
                tasks.map((task: any) => (
                  <div key={task.id} className="group flex items-center justify-between p-3 rounded-xl border border-gray-200 bg-white shadow-sm hover:border-gray-300 transition-colors">
                ...
                <div className="text-center py-10">
                  <p className="text-sm text-gray-400 italic">Nenhuma tarefa aberta neste projeto.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm h-fit space-y-6">
            <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4">Detalhes do Projeto</h2>
            ...
            <div className="pt-4 border-t border-gray-100 flex justify-end">
              <button
                ...
                className="px-6 py-2 bg-[#DC4C3E] text-white text-sm font-bold rounded-lg hover:bg-[#C53727] transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm flex items-center gap-2"
              >
